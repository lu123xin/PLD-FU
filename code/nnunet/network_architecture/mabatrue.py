...
class nnMambaSeg(SegmentationNetwork):
    def __init__(self, in_ch=3, channels=32, blocks=3, number_classes=2):#in_ch: 输入通道数，通常为1（灰度图像）或3（RGB图像）。channels: 初始通道数，默认值为32，用于卷积操作。blocks: 每个残差块的数量，默认值为3。
