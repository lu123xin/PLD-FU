...
def augment_volume(volume):
    ...
    volume_aug = volume.clone()
    if random.random() > 0.5:
        volume_aug = torch.flip(volume_aug, dims=[3])  #
    if random.random() > 0.5:
        volume_aug = torch.rot90(volume_aug, k=1, dims=[3, 4])  
    return volume_aug
...
if __name__ == "__main__":
    ...
    model = net_factory(net_type=args.model, in_chns=1, class_num=num_classes, mode="train").cuda()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for num in range(3):
                ...
                weighted_outputs = weights_1.unsqueeze(1) * y_all[0] + weights_2.unsqueeze(1) * y_all[1]
                ...
                for i in range(num_outputs):
                    for j in range(num_outputs):
                        if i != j:
                            ...
                            weight_pixel_t1 = nn.MSELoss(reduction='none')(mask_t, y_all[j]).mean(1)
                ...
                volume_batch_aug = augment_volume(volume_batch)

                model.eval()
                with torch.no_grad():
                    outputs_aug, _, _, _ = model(volume_batch_aug, en if 'en' in locals() else [])
                model.train()

                y_prob_aug = F.softmax(outputs_aug[0], dim=1)
                y_prob_orig = F.softmax(outputs[0], dim=1)

                _, pseudo_outputs_orig = torch.max(y_prob_orig, dim=1)
                _, pseudo_outputs_aug = torch.max(y_prob_aug, dim=1)
                conflict_mask = (pseudo_outputs_orig != pseudo_outputs_aug).float()

                entropy_orig = -torch.sum(y_prob_orig * torch.log(y_prob_orig + 1e-10), dim=1)
                weights = torch.exp(-entropy_orig) * conflict_mask

                consistency_diff = (y_prob_aug - y_prob_orig).pow(2).sum(dim=1)
                loss_consistency_reg = (weights * consistency_diff).sum() / (weights.sum() + 1e-6)

                consistency_weight_current = get_current_consistency_weight(iter_num // 150)

                loss = args.lamda * loss_seg_dice + consistency_weight_current * (loss_consist + loss_conflict + loss_consistency_reg) + args.lamda * los
            ...
            iter_num += 1
            logging.info('iteration %d : loss : %.6f, loss_d: %.6f, loss_consist: %.6f, los_ds: %.6f, loss_conflict: %.6f, loss_consistency_reg: %.6f' %
                         (iter_num, loss.item(), loss_seg_dice.item(), loss_consist.item(), los.item(), loss_conflict.item(), loss_consistency_reg.item()))

            writer.add_scalar('Labeled_loss/loss_seg_dice', loss_seg_dice.item(), iter_num)
            writer.add_scalar('Co_loss/consistency_loss', loss_consist.item(), iter_num)
            writer.add_scalar('Co_loss/consistency_reg', loss_consistency_reg.item(), iter_num)
