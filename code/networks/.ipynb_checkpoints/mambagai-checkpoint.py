...
class SideConv(nn.Module):
    ...
    def forward(self, stage_feat):
        x5, x5_up, x6_up, x7_up = stage_feat[0], stage_feat[1], stage_feat[2], stage_feat[3]
        out5 = self.side5(x5)
...
class nnMambaSegSL(nn.Module):
    ...
    def forward(self, x,en=[]):
        if len(en)!=0:
            c1 = self.in_conv(x)
            print('c1',c1.shape)
            print('en[3]',en[3].shape)
            c1 = c1 + en[3]
            scale_f1 = self.att1(self.pooling(c1).reshape(c1.shape[0], c1.shape[1])).reshape(c1.shape[0], c1.shape[1],
                                                                                             1, 1,
                                                                                             1)
            # c1_s = self.mamba_layer_stem(c1) + c1
            c2 = self.layer1(c1)
            c2=c2+en[2]
            # c2_s = self.mamba_layer_1(c2) + c2
            scale_f2 = self.att2(self.pooling(c2).reshape(c2.shape[0], c2.shape[1])).reshape(c2.shape[0], c2.shape[1],
                                                                                             1, 1,
                                                                                             1)
            ...
            c3=c3+en[1]
            # c3_s = self.mamba_layer_2(c3) + c3
            scale_f3 = self.att3(self.pooling(c3).reshape(c3.shape[0], c3.shape[1])).reshape(c3.shape[0], c3.shape[1],
                                                                                             1, 1,
                                                                                             1)
            c4 = self.layer3(c3)
            c4=c4+en[0]
        ...
        ls2=[c4_2,c5_2,c6_2,c7_2]

        return [c8, c8_2], [up_8, up_8_2],[ls,ls2],deep_out1
