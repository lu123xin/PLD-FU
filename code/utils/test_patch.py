...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open('/mnt/workspace/mutual-learning-with-reliable-pseudo-labels/data/LA/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = [
            "/mnt/workspace/mutual-learning-with-reliable-pseudo-labels/data/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5"
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    ...
                    #for i in y:
                        ...
                        #print("list的长度",len(i))
                        #print(type(i[0]))
#                     max_length = max(len(lst) for lst in y)

# # 填充短列表，使它们的长度相同
#                     y_padded = [lst + [0] * (max_length - len(lst)) for lst in y]
#                     y_tensors = tuple(torch.tensor(lst) for lst in y_padded)
                    #stacked_tensor_along_columns = torch.stack(y_tensors, dim=1)
                    # y_cpu = tuple(t.cpu().numpy() for t in y) 
                    #y_np = np.array(y_cpu)
                    #print(type(y))
                    #print(type(y))
                    #y_tensor = torch.tensor(y_cpu)
                    if len(y) > 1:
                        # change here
                        y = y [0][0] #gai
                        #print(type(y))
...
def test_single_case_average_output(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    sz = math.ceil((dd - patch_size[2]) / stride_z) + 1
    # print("{}, {}, {}".format(sx, sy, sz))
    score_map = np.zeros((num_classes, ) + image.shape).astype(float32)
    cnt = np.zeros(image.shape).astype(float32)
